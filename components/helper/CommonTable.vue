<template>
    <div class="w-full h-full custom-table-wrapper" :style="{ width: 100 - uiStore.sizeBarWidth - 4 + 'vw' }">
        <a-table class="flex-1 w-full" ref="tableRef" size="small" :rowKey="_rowkey" :pagination="false"
            :dataSource="dataSource" :columns="columns" :scroll="{ x: lenghtX, y: lenghtY }"
            :rowClassName="(_, index) => index === selectedIndex ? 'selected-row' : 'cursor-pointer'" :customRow="(record, index) => ({
                onClick: () => {
                    selectedIndex = index;
...
const props = withDefaults(defineProps<{
    ...
    rowClassName: () => () => '',
    lenghtX: 1000,
    lenghtY: 'calc((100vh - 26rem - 12px)/2)',
