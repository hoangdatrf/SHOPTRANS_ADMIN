...
<div class="s5-root">
        <!-- ── STEP 5 Tab bar ── -->
        <div class="service-tabs-bar s5-tab-bar" v-if="step5TabList.length > 0">
          <!-- AT ORIGIN group -->
        ...
        <div class="s5-mc">
          ...
          <!-- ── SHIPPING BOOKING (only when Handle under available booking IS selected — KO DÙNG SHOPTRANS) ── -->
          ...
          <div class="s5-sb"><div class="s5-fg">
            ...
            <div class="s5-row-ship">
              <div class="s5-ff">
                <span class="s5-fl">Shipping line <span class="s5-rq">*</span></span>
                <select class="s5-fi s5-fi-select custom-caret-svg" :class="{ 's5-inv': step5Errors.has('fca.shippingLine') }" v-model="fcaBooking.shippingLine" @change="step5Errors.delete('fca.shippingLine')">
                  ...
                  <option>COSCO</option><option>HAPAG-LLOYD</option><option>ONE</option>
                  <option>EVERGREEN</option><option>HMM</option><option>YANG MING</option>
                  <option>ZIM</option><option>PIL</option><option>OOCL</option><option>WAN HAI</option>
                </select>
              </div>
              <div class="s5-ff">
          ...
          <div class="s5-sb"><div class="s5-fg">
            ...
            <div class="s5-sar" :class="{ act: fcaBooking.sameAsShipper }" @click="fcaBooking.sameAsShipper = !fcaBooking.sameAsShipper">
          ...
          <!-- ── DOCUMENT TO USE (only when handleUnderAvailable — KO DÙNG SHOPTRANS) ── -->
          <template v-if="services.sea.selected.includes('handleUnderAvailable')">
          <div class="s5-sd">
            <div class="s5-si">
            ...
            <span class="s5-st">Document to Use</span>
          </div>
          <div class="s5-sb"><div class="s5-fg">

            <!-- Upload mode toggle -->
            <div class="s5-ut">
              <div class="s5-uo" :class="{ act: fcaBooking.uploadType === 'compressed' }" @click="fcaBooking.uploadType = 'compressed'">
                <span class="s5-uor"></span>
                <div class="s5-uo-body">
                  <span class="s5-uot">Upload all documents in one compressed file</span>
                  <span class="s5-uoh">Support: .rar, .zip, .7z</span>
                </div>
              </div>
              <div class="s5-uo" :class="{ act: fcaBooking.uploadType === 'separate' }" @click="fcaBooking.uploadType = 'separate'">
                <span class="s5-uor"></span>
                <div class="s5-uo-body">
                  <span class="s5-uot">Upload each document separately</span>
                  <span class="s5-uoh">Support: PDF, XLSX, DOCX,...</span>
                </div>
              </div>
            </div>

            <!-- File upload zone -->
            <div class="s5-upload-zone">
              <label class="s5-frow">
                <span class="s5-fri">{{ fcaBooking.uploadType === 'compressed' ? '📦' : '📄' }}</span>
                <div class="s5-frn">
                  <div class="s5-frn-t">{{ fcaBooking.uploadType === 'compressed' ? 'All_Documents.zip' : 'Booking Order' }}</div>
                  <div class="s5-frn-s">{{ fcaBooking.compressedFiles.length ? '✓ ' + fcaBooking.compressedFiles[0].name : 'Click anywhere to upload' }}</div>
                </div>
                <span class="s5-frt s5-frt-rq">Required</span>
                <input type="file" style="display:none" :accept="fcaBooking.uploadType === 'compressed' ? '.rar,.zip,.7z' : '.pdf,.xlsx,.xls,.docx,.doc,.jpg,.jpeg,.png'" @change="(e: Event) => { const f = (e.target as HTMLInputElement).files?.[0]; if(f) fcaBooking.compressedFiles = [f] }" />
              </label>
              <template v-if="fcaBooking.uploadType === 'separate'">
                <label class="s5-frow">
                  <span class="s5-fri">📋</span>
                  <div class="s5-frn">
                    <div class="s5-frn-t">VGM</div>
                    <div class="s5-frn-s">{{ fcaBooking.separateFiles.length > 1 ? '✓ ' + fcaBooking.separateFiles[1].name : 'Click anywhere to upload' }}</div>
                  </div>
                  <span class="s5-frt s5-frt-rq">Required</span>
                  <input type="file" style="display:none" accept=".pdf,.xlsx,.xls,.docx,.doc,.jpg,.jpeg,.png" @change="(e: Event) => { const f = (e.target as HTMLInputElement).files?.[0]; if(f) { const arr = [...fcaBooking.separateFiles]; arr[1] = f; fcaBooking.separateFiles = arr } }" />
                </label>
              </template>
            </div>

          </div></div>
          </template><!-- end document to use block -->
                  ...
                  <div class="s5-upload-step-body">
                    ...
                    <div class="s5-dal-chips">
                      <div class="s5-dal-chip" :class="{ act: customsBooking.docAllocation === 'same' }" @click="customsBooking.docAllocation = 'same'">
                        ...
                        <div>
                          <div class="s5-dal-chip-t">Same document set</div>
                          <div class="s5-dal-chip-h">All containers share one set</div>
                        </div>
                      </div>
                      <div class="s5-dal-chip" :class="{ act: customsBooking.docAllocation === 'each' }" @click="customsBooking.docAllocation = 'each'">
                        <span class="s5-dal-chip-r"></span>
                        <div>
                          <div class="s5-dal-chip-t">Separate per container</div>
                          <div class="s5-dal-chip-h">Each container has its own document set</div>
                        </div>
                      </div>
                      <div class="s5-dal-chip" :class="{ act: customsBooking.docAllocation === 'multi' }" @click="customsBooking.docAllocation = 'multi'">
                        <span class="s5-dal-chip-r"></span>
                        <div>
                          <div class="s5-dal-chip-t">Other scenarios</div>
                          <div class="s5-dal-chip-h">One or more containers use multiple document sets</div>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div class="s5-sb">
            ...
            <div v-if="collectPayBooking.collectionSubmitType === 'MANUAL'" class="s5-cp-fee-wrap">
              <div class="s5-cp-fee-actions">
                <button class="s5-cp-fee-btn s5-cp-fee-add" :class="{ 's5-inv': step5Errors.has('cp.collection.fees') }" @click="step5Errors.delete('cp.collection.fees'); collectPayBooking.collectionFees.push({ chargeName:'Ocean Freight', unit:'CNT', quantity:'', unitPrice:'', currency:'USD', total:'', receiveFrom:'Shipper' })">
                ...
                <table class="s5-cp-table">
                  ...
                  <tbody>
                    <tr v-for="(row, i) in collectPayBooking.collectionFees" :key="i">
                      <td><select class="s5-cp-tc-sel custom-caret-svg" :class="{ 's5-inv': step5Errors.has('cp.collection.fees.' + i + '.chargeName') }" v-model="row.chargeName" @change="step5Errors.delete('cp.collection.fees.' + i + '.chargeName')"><option v-for="cn in cpChargeNameOpts" :key="cn" :value="cn">{{ cn }}</option></select></td>
                      <td><select class="s5-cp-tc-sel custom-caret-svg" :class="{ 's5-inv': step5Errors.has('cp.collection.fees.' + i + '.unit') }" v-model="row.unit" @change="step5Errors.delete('cp.collection.fees.' + i + '.unit')"><option v-for="u in cpUnitOpts" :key="u" :value="u">{{ u }}</option></select></td>
                      <td><input type="number" class="s5-cp-tc" :class="{ 's5-inv': step5Errors.has('cp.collection.fees.' + i + '.quantity') }" v-model="row.quantity" @input="step5Errors.delete('cp.collection.fees.' + i + '.quantity')" placeholder="1" min="0.01" step="any" /></td>
                      <td><input type="number" class="s5-cp-tc" :class="{ 's5-inv': step5Errors.has('cp.collection.fees.' + i + '.unitPrice') }" v-model="row.unitPrice" @input="step5Errors.delete('cp.collection.fees.' + i + '.unitPrice')" placeholder="0.00" min="0" step="0.01" /></td>
                      <td><select class="s5-cp-tc-sel custom-caret-svg" :class="{ 's5-inv': step5Errors.has('cp.collection.fees.' + i + '.currency') }" v-model="row.currency" @change="step5Errors.delete('cp.collection.fees.' + i + '.currency')"><option>USD</option><option>EUR</option><option>VND</option><option>CNY</option><option>JPY</option><option>GBP</option></select></td>
            ...
            <div v-if="collectPayBooking.collectionSubmitType !== 'LATER'" class="s5-cp-pic-section s5-cp-pic-green">
              ...
              <div v-for="(pic, pi) in cpColPics" :key="'cpic-c-'+pi" class="s5-cp-pic-row" :style="pi > 0 ? 'margin-top:8px' : ''">
                ...
                <div class="s5-cp-pic-field">
                  ...
                  <input class="s5-cp-pic-fi" :class="{ 's5-inv': step5Errors.has('cp.collection.pics.' + pi + '.name') }" type="text" v-model="pic.name" @input="step5Errors.delete('cp.collection.pics.' + pi + '.name')" placeholder="Input PIC name" />
                </div>
                <div class="s5-cp-pic-field">
                  <label class="s5-cp-pic-fl">Email</label>
                  <input class="s5-cp-pic-fi" :class="{ 's5-inv': step5Errors.has('cp.collection.pics.' + pi + '.email') }" type="email" v-model="pic.email" @input="step5Errors.delete('cp.collection.pics.' + pi + '.email')" placeholder="Input Email address" />
                </div>
                <div class="s5-cp-pic-field">
                  <label class="s5-cp-pic-fl">Phone</label>
                  <input class="s5-cp-pic-fi" :class="{ 's5-inv': step5Errors.has('cp.collection.pics.' + pi + '.phone') }" type="tel" v-model="pic.phone" @input="step5Errors.delete('cp.collection.pics.' + pi + '.phone')" placeholder="Input phone number" maxlength="20" />
                </div>
        ...
        <div class="s5-mc s5-mc-pay">
          ...
          <div class="s5-sb">
            ...
            <div v-if="collectPayBooking.paymentSubmitType === 'MANUAL'" class="s5-cp-fee-wrap">
              <div class="s5-cp-fee-actions">
                <button class="s5-cp-fee-btn s5-cp-fee-add" :class="{ 's5-inv': step5Errors.has('cp.payment.fees') }" @click="step5Errors.delete('cp.payment.fees'); collectPayBooking.paymentFees.push({ chargeName:'Ocean Freight', unit:'CNT', quantity:'', unitPrice:'', currency:'USD', total:'', payTo:'Shipper' })">
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg> ADD
                </button>
              </div>
              <div class="s5-cp-table-wrap">
                <table class="s5-cp-table">
                  ...
                  <tbody>
                    <tr v-for="(row, i) in collectPayBooking.paymentFees" :key="i">
                      <td><select class="s5-cp-tc-sel custom-caret-svg" :class="{ 's5-inv': step5Errors.has('cp.payment.fees.' + i + '.chargeName') }" v-model="row.chargeName" @change="step5Errors.delete('cp.payment.fees.' + i + '.chargeName')"><option v-for="cn in cpChargeNameOpts" :key="cn" :value="cn">{{ cn }}</option></select></td>
                      <td><select class="s5-cp-tc-sel custom-caret-svg" :class="{ 's5-inv': step5Errors.has('cp.payment.fees.' + i + '.unit') }" v-model="row.unit" @change="step5Errors.delete('cp.payment.fees.' + i + '.unit')"><option v-for="u in cpUnitOpts" :key="u" :value="u">{{ u }}</option></select></td>
                      <td><input type="number" class="s5-cp-tc" :class="{ 's5-inv': step5Errors.has('cp.payment.fees.' + i + '.quantity') }" v-model="row.quantity" @input="step5Errors.delete('cp.payment.fees.' + i + '.quantity')" placeholder="1" min="0.01" step="any" /></td>
                      <td><input type="number" class="s5-cp-tc" :class="{ 's5-inv': step5Errors.has('cp.payment.fees.' + i + '.unitPrice') }" v-model="row.unitPrice" @input="step5Errors.delete('cp.payment.fees.' + i + '.unitPrice')" placeholder="0.00" min="0" step="0.01" /></td>
                      <td><select class="s5-cp-tc-sel custom-caret-svg" :class="{ 's5-inv': step5Errors.has('cp.payment.fees.' + i + '.currency') }" v-model="row.currency" @change="step5Errors.delete('cp.payment.fees.' + i + '.currency')"><option>USD</option><option>EUR</option><option>VND</option><option>CNY</option><option>JPY</option><option>GBP</option></select></td>
                      <td><div class="s5-cp-tc-total">{{ row.quantity && row.unitPrice ? (parseFloat(row.quantity)*parseFloat(row.unitPrice)).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+' '+row.currency : '—' }}</div></td>
                      <td>
            ...
            <div v-if="collectPayBooking.paymentSubmitType !== 'LATER'" class="s5-cp-pic-section s5-cp-pic-red">
              ...
              <div v-for="(pic, pi) in cpPayPics" :key="'cpic-p-'+pi" class="s5-cp-pic-row" :style="pi > 0 ? 'margin-top:8px' : ''">
                ...
                <div class="s5-cp-pic-field">
                  <label class="s5-cp-pic-fl">PIC name</label>
                  <input class="s5-cp-pic-fi" :class="{ 's5-inv': step5Errors.has('cp.payment.pics.' + pi + '.name') }" type="text" v-model="pic.name" @input="step5Errors.delete('cp.payment.pics.' + pi + '.name')" placeholder="Input PIC name" />
                </div>
                <div class="s5-cp-pic-field">
                  <label class="s5-cp-pic-fl">Email</label>
                  <input class="s5-cp-pic-fi" :class="{ 's5-inv': step5Errors.has('cp.payment.pics.' + pi + '.email') }" type="email" v-model="pic.email" @input="step5Errors.delete('cp.payment.pics.' + pi + '.email')" placeholder="Input Email address" />
                </div>
                <div class="s5-cp-pic-field">
                  <label class="s5-cp-pic-fl">Phone</label>
                  <input class="s5-cp-pic-fi" :class="{ 's5-inv': step5Errors.has('cp.payment.pics.' + pi + '.phone') }" type="tel" v-model="pic.phone" @input="step5Errors.delete('cp.payment.pics.' + pi + '.phone')" placeholder="Input phone number" maxlength="20" />
                </div>
                <div class="s5-cp-pic-field s5-cp-pic-btn-cell">
        ...
        <div class="s5-mc">
          ...
          <div class="s5-sb"><div class="s5-fg">
            ...
            <!-- Same as shipper toggle -->
            <div class="s5-sar" :class="{ act: exwBooking.sameAsShipper }" @click="exwBooking.sameAsShipper = !exwBooking.sameAsShipper">
              <span class="s5-sc"></span>
              <span class="s5-sal">SAME AS SHIPPER</span>
            </div>
              ...
              <div class="s5-upload-step">
                ...
                <div class="s5-upload-step-body">
                  ...
                  <div class="s5-dal-chips">
                    ...
                    <div class="s5-dal-chip" :class="{ act: exwBooking.docAllocation === 'each' }" @click="exwBooking.docAllocation = 'each'">
                      <span class="s5-dal-chip-r"></span>
                      <div>
                        <div class="s5-dal-chip-t">Separate per container</div>
                        <div class="s5-dal-chip-h">Each container has its own document set</div>
        ...
        <div class="s5-mc">
          <div class="s5-sd s5-sd-flex">
            <div class="s5-sd-left">
              ...
              <span class="s5-st">Offered Rate</span>
            </div>
          ...
          <div class="s5-or-rate-grid">
            ...
            <div class="s5-or-rate-item">
              <div class="s5-or-rate-label">Freetime</div>
          ...
          <div class="s5-av-bk-grid">
            ...
            <div class="s5-av-bk-field">
              ...
              <div class="s5-av-chip-zone">
                <template v-for="(file, idx) in availableBooking.confirmFiles" :key="idx">
                  <div class="s5-av-chip-row">
                    <div class="s5-av-chip">
                      <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#6b7280" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                      <span class="s5-av-chip-name">{{ file.name }}</span>
                      <button class="s5-av-chip-del" @click.stop="availableBooking.confirmFiles.splice(idx,1)">×</button>
                    </div>
                  </div>
                </template>
                <div class="s5-av-add-row">
                  <label class="s5-av-add-btn">
                    <input type="file" multiple accept=".rar,.zip,.7z,.pdf,.xlsx,.xls,.docx,.doc,.png,.jpg" style="display:none" @change="(e: Event) => { const files = (e.target as HTMLInputElement).files; if(files) Array.from(files).forEach(f => availableBooking.confirmFiles.push(f)); (e.target as HTMLInputElement).value = '' }">
              ...
              <div class="s5-sc-info-grid">
                <div class="s5-av-bk-field">
                  ...
                  <select class="s5-or-bk-sel" v-model="contractBooking.shippingLine">
                    <option value="">Select carrier</option>
                    <option>Evergreen Line</option><option>COSCO</option><option>MSC</option>
                    <option>CMA CGM</option><option>Hapag-Lloyd</option><option>ONE</option>
                    <option>Yang Ming</option><option>HMM</option><option>ZIM</option>
                  </select>
                </div>
                <div class="s5-av-bk-field">
              ...
              <div class="s5-av-bk-field">
                ...
                <div class="s5-av-chip-zone">
                  <template v-for="(item, idx) in contractBooking.confirmFiles" :key="idx">
                    <div class="s5-bk-chip" :class="{ 'up': true }">
                      <!-- Tick icon (uploaded) -->
                      ...
                      <!-- Editable name input -->
                      <input type="text" class="s5-bk-chip-inp" v-model="item.displayName" :class="{ first: idx === 0 }" @click.stop @keydown.enter="($event.target as HTMLInputElement).blur()" :title="item.file.name" />
                      <!-- Green tick circle -->
                  ...
                  <div class="s5-av-add-row">
                    <label class="s5-av-add-btn">
                      <input type="file" multiple accept=".rar,.zip,.7z,.pdf,.xlsx,.xls,.docx,.doc,.png,.jpg" style="display:none" @change="(e: Event) => { const files = (e.target as HTMLInputElement).files; if(files) Array.from(files).forEach(f => contractBooking.confirmFiles.push({file:f, displayName: contractBooking.confirmFiles.length === 0 ? 'Booking Confirmation' : f.name.replace(/\.[^.]+$/,'')})); (e.target as HTMLInputElement).value = '' }">
                      <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                      Add file
                    </label>
                  </div>
          ...
          <div class="s5-mc" style="margin-bottom:16px" v-if="services.sea.selected.includes('handleUnderAvailable')">
            ...
            <div style="padding:20px 24px;">
              <div class="s5-sbl-mbl-grid">
                <div class="s5-av-bk-field">
                  <div class="s5-or-bk-fl">Shipping line</div>
                  <select class="s5-or-bk-sel" v-model="switchBillBooking.shippingLine">
                    <option value="">Select carrier</option>
                    <option>COSCO</option><option>Evergreen</option><option>MSC</option>
                    <option>Maersk</option><option>CMA CGM</option><option>ONE</option>
                    <option>Yang Ming</option><option>HMM</option><option>ZIM</option>
                  </select>
                </div>
                <div class="s5-av-bk-field">
        ...
        <div v-show="activeStep5Tab === 's5-dest-dap'" class="s5-page">
          ...
          <div class="s5-mc" style="margin-bottom:16px">
            ...
            <div class="s5-sb"><div class="s5-fg">
              ...
              <div class="s5-ff">
                ...
                <input type="text" class="s5-fi" :class="{ 's5-inv': step5Errors.has('dap.receivingCompany'), 's5-disabled': dapBooking.sameAsConsignee }" v-model="dapBooking.receivingCompany" :disabled="dapBooking.sameAsConsignee" @input="step5Errors.delete('dap.receivingCompany')" placeholder="Enter company name" />
              </div>

              <div class="s5-row-addr-zip">
                <div class="s5-ff">
                  <span class="s5-fl">Company address <span class="s5-rq">*</span></span>
                  <input type="text" class="s5-fi" :class="{ 's5-inv': step5Errors.has('dap.companyAddress'), 's5-disabled': dapBooking.sameAsConsignee }" v-model="dapBooking.companyAddress" :disabled="dapBooking.sameAsConsignee" @input="step5Errors.delete('dap.companyAddress')" placeholder="Full delivery address including city, state, country" />
                </div>
                <div class="s5-ff">
                  <span class="s5-fl">Zip code <span class="s5-rq">*</span></span>
                  <input type="text" class="s5-fi" :class="{ 's5-inv': step5Errors.has('dap.zipCode'), 's5-disabled': dapBooking.sameAsConsignee }" v-model="dapBooking.zipCode" :disabled="dapBooking.sameAsConsignee" @input="step5Errors.delete('dap.zipCode')" placeholder="e.g. 700000" maxlength="10" />
                </div>
              </div>
              ...
              <div class="s5-row-pic">
                <div class="s5-ff">
                  <span class="s5-fl">Title</span>
                  <select class="s5-fi s5-fi-select custom-caret-svg" :class="{ 's5-disabled': dapBooking.sameAsConsignee }" v-model="dapBooking.picTitle" :disabled="dapBooking.sameAsConsignee">
                    <option>Mr.</option><option>Mrs.</option><option>Ms.</option>
                  </select>
                </div>
                <div class="s5-ff">
                  <span class="s5-fl">PIC name <span class="s5-rq">*</span></span>
                  <input type="text" class="s5-fi" :class="{ 's5-inv': step5Errors.has('dap.picName'), 's5-disabled': dapBooking.sameAsConsignee }" v-model="dapBooking.picName" :disabled="dapBooking.sameAsConsignee" @input="step5Errors.delete('dap.picName')" placeholder="Contact person in charge" />
                </div>
                <div class="s5-ff">
                  <span class="s5-fl">Email <span class="s5-rq">*</span></span>
                  <input type="text" class="s5-fi" :class="{ 's5-inv': step5Errors.has('dap.email'), 's5-disabled': dapBooking.sameAsConsignee }" v-model="dapBooking.email" :disabled="dapBooking.sameAsConsignee" @input="step5Errors.delete('dap.email')" placeholder="email@company.com" />
                </div>
                <div class="s5-ff">
                  <span class="s5-fl">Phone <span class="s5-rq">*</span></span>
                  <input type="tel" class="s5-fi" :class="{ 's5-inv': step5Errors.has('dap.phone'), 's5-disabled': dapBooking.sameAsConsignee }" v-model="dapBooking.phone" :disabled="dapBooking.sameAsConsignee" @input="step5Errors.delete('dap.phone')" placeholder="e.g. +84 28 1234 567" maxlength="18" />
                </div>
              </div>
          ...
          <div class="s5-mc" style="margin-bottom:16px" v-if="services.sea.selected.includes('handleUnderAvailable')">
            ...
            <div class="s5-sb"><div class="s5-fg">
              ...
              <!-- Row 1: Shipping line / B/L number / Vessel name / Voyage number / Est. Arrival time -->
              <div style="display:grid;grid-template-columns:140px 1fr 1.4fr 1.2fr 160px;gap:10px;align-items:end;">
                <div class="s5-ff">
                  <span class="s5-fl">Shipping line</span>
              ...
              <div style="display:grid;grid-template-columns:160px 280px 1fr;gap:10px;align-items:start;">
                ...
                <!-- Freetime application -->
                <div style="border:1.5px solid var(--s5-border,#e5e7eb);border-radius:10px;padding:10px 14px;">
                  <div class="s5-fl" style="margin-bottom:10px;">Freetime application</div>
                  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;">
                    <div>
                      <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Demurrage</div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input type="number" class="s5-fi" v-model="dapBooking.arrFreeDemurrage" placeholder="e.g. 14" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" />
                        <span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span>
                      </div>
                    </div>
                    <div>
                      <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Detention</div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input type="number" class="s5-fi" v-model="dapBooking.arrFreeDetention" placeholder="e.g. 7" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" />
                        <span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span>
                      </div>
                    </div>
                    <div>
                      <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Combine</div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input type="number" class="s5-fi" v-model="dapBooking.arrFreeCombine" placeholder="e.g. 21" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" />
                        <span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span>
                      </div>
                    </div>
                  </div>
          ...
          <!-- ── ATTACHMENTS (only when combined with handleUnderAvailable) ── -->
          <div class="s5-mc" style="margin-bottom:16px" v-if="services.sea.selected.includes('handleUnderAvailable')">
            <div class="s5-sd" style="border-radius:14px 14px 0 0">
              <div class="s5-sd-left">
            ...
            <div class="s5-sb"><div class="s5-fg">
              ...
              <div class="s5-upload-step">
                ...
                <div class="s5-upload-step-body">
                  <div class="s5-dal-label">Document Set Allocation</div>
                  <div class="s5-dal-chips">
                    <div class="s5-dal-chip" :class="{ 'act': dapBooking.docAlloc === 'same' }" @click="dapBooking.docAlloc = 'same'">
                      <span class="s5-dal-chip-r"></span>
                      <div><div class="s5-dal-chip-t">Same document set</div><div class="s5-dal-chip-h">All containers share one set</div></div>
                    </div>
                    <div class="s5-dal-chip" :class="{ 'act': dapBooking.docAlloc === 'each' }" @click="dapBooking.docAlloc = 'each'">
                      <span class="s5-dal-chip-r"></span>
                    ...
                    <div class="s5-dal-chip" :class="{ 'act': dapBooking.docAlloc === 'multi' }" @click="dapBooking.docAlloc = 'multi'">
                      <span class="s5-dal-chip-r"></span>
              ...
              <div class="s5-upload-step">
                ...
                <div class="s5-upload-step-body">
                  ...
                  <div v-if="dapBooking.uploadMode === 'compressed'" class="s5-upload-zone">
                    ...
                    <button v-if="dapBooking.docAlloc !== 'same'" class="s5-afb" @click="addDapRow('compressed')" type="button">
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                      Add more
                    </button>
                  </div>
              ...
              <div class="s5-upload-step" style="border-bottom:none;">
                <div class="s5-upload-step-num">③</div>
                <div class="s5-upload-step-body">
                  <div class="s5-dcl" :class="{ 'chk': dapBooking.checkDocs }" @click="dapBooking.checkDocs = !dapBooking.checkDocs">
                    <span class="s5-dcb"></span>
                    <span class="s5-dct">
                  ...
                  <div class="s5-dcl" :class="{ 'chk': dapBooking.checkCO }" @click="dapBooking.checkCO = !dapBooking.checkCO" style="margin-top:8px">
                    <span class="s5-dcb"></span>
          ...
          <div class="s5-ba">
            ...
            <button class="s5-btn s5-btn-next" :disabled="services.sea.selected.includes('handleUnderAvailable') && !dapBooking.checkDocs" @click="validateStep5">
              Continue
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
            </button>
          </div>
        ...
        <div v-show="activeStep5Tab === 's5-dest-ddu'" class="s5-page">
          ...
          <div class="s5-mc" style="margin-bottom:16px">
            ...
            <div class="s5-sb"><div class="s5-fg">
              ...
              <div class="s5-ff">
                <span class="s5-fl">Receiving company <span class="s5-rq">*</span></span>
                <input type="text" class="s5-fi" :class="{ 's5-inv': step5Errors.has('ddu.receivingCompany'), 's5-disabled': dduBooking.sameAsConsignee }" v-model="dduBooking.receivingCompany" :disabled="dduBooking.sameAsConsignee" @input="step5Errors.delete('ddu.receivingCompany')" placeholder="Enter company name" />
              </div>

              <div class="s5-row-addr-zip">
                <div class="s5-ff">
                  <span class="s5-fl">Company address <span class="s5-rq">*</span></span>
                  <input type="text" class="s5-fi" :class="{ 's5-inv': step5Errors.has('ddu.companyAddress'), 's5-disabled': dduBooking.sameAsConsignee }" v-model="dduBooking.companyAddress" :disabled="dduBooking.sameAsConsignee" @input="step5Errors.delete('ddu.companyAddress')" placeholder="Full delivery address including city, state, country" />
                </div>
                <div class="s5-ff">
                  <span class="s5-fl">Zip code <span class="s5-rq">*</span></span>
                  <input type="text" class="s5-fi" :class="{ 's5-inv': step5Errors.has('ddu.zipCode'), 's5-disabled': dduBooking.sameAsConsignee }" v-model="dduBooking.zipCode" :disabled="dduBooking.sameAsConsignee" @input="step5Errors.delete('ddu.zipCode')" placeholder="e.g. 700000" maxlength="10" />
                </div>
              </div>
              ...
              <div class="s5-row-pic">
                <div class="s5-ff">
                  <span class="s5-fl">Title</span>
                  <select class="s5-fi s5-fi-select custom-caret-svg" :class="{ 's5-disabled': dduBooking.sameAsConsignee }" v-model="dduBooking.picTitle" :disabled="dduBooking.sameAsConsignee">
                    <option>Mr.</option><option>Mrs.</option><option>Ms.</option>
                  </select>
                </div>
                <div class="s5-ff">
                  <span class="s5-fl">PIC name <span class="s5-rq">*</span></span>
                  <input type="text" class="s5-fi" :class="{ 's5-inv': step5Errors.has('ddu.picName'), 's5-disabled': dduBooking.sameAsConsignee }" v-model="dduBooking.picName" :disabled="dduBooking.sameAsConsignee" @input="step5Errors.delete('ddu.picName')" placeholder="Contact person in charge" />
                </div>
                <div class="s5-ff">
                  <span class="s5-fl">Email <span class="s5-rq">*</span></span>
                  <input type="text" class="s5-fi" :class="{ 's5-inv': step5Errors.has('ddu.email'), 's5-disabled': dduBooking.sameAsConsignee }" v-model="dduBooking.email" :disabled="dduBooking.sameAsConsignee" @input="step5Errors.delete('ddu.email')" placeholder="email@company.com" />
                </div>
                <div class="s5-ff">
                  <span class="s5-fl">Phone <span class="s5-rq">*</span></span>
                  <input type="tel" class="s5-fi" :class="{ 's5-inv': step5Errors.has('ddu.phone'), 's5-disabled': dduBooking.sameAsConsignee }" v-model="dduBooking.phone" :disabled="dduBooking.sameAsConsignee" @input="step5Errors.delete('ddu.phone')" placeholder="e.g. +84 28 1234 567" maxlength="18" />
                </div>
              </div>
            ...
            <div class="s5-sb"><div class="s5-fg">
              ...
              <div style="display:grid;grid-template-columns:160px 280px 1fr;gap:10px;align-items:start;">
                ...
                <div style="border:1.5px solid var(--s5-border,#e5e7eb);border-radius:10px;padding:10px 14px;">
                  <div class="s5-fl" style="margin-bottom:10px;">Freetime application</div>
                  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;">
                    <div><div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Demurrage</div><div style="display:flex;align-items:center;gap:6px;"><input type="number" class="s5-fi" v-model="dduBooking.arrFreeDemurrage" placeholder="e.g. 14" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" /><span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span></div></div>
                    <div><div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Detention</div><div style="display:flex;align-items:center;gap:6px;"><input type="number" class="s5-fi" v-model="dduBooking.arrFreeDetention" placeholder="e.g. 7" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" /><span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span></div></div>
                    <div><div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Combine</div><div style="display:flex;align-items:center;gap:6px;"><input type="number" class="s5-fi" v-model="dduBooking.arrFreeCombine" placeholder="e.g. 21" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" /><span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span></div></div>
                  </div>
                </div>
              </div>
            </div></div>
                ...
                <div class="s5-cop" :class="{ 's5-cop-sel': dduBooking.customsOption === 1 }" @click="dduBooking.customsOption = 1">
                  ...
                  <div class="s5-cop-pb" v-if="dduBooking.customsOption === 1">
                    ...
                    <select class="s5-fi s5-fi-select custom-caret-svg" v-model="dduBooking.customsProvidedBy" @click.stop style="width:auto;padding:6px 32px 6px 10px;font-size:12px;">
                      <option>Consignee</option><option>Our company</option><option>Third party</option>
                    </select>
                  </div>
                </div>
                ...
                <div class="s5-upload-step">
                  ...
                  <div class="s5-upload-step-body">
                    ...
                    <div class="s5-dal-chips">
                      ...
                      <div class="s5-dal-chip" :class="{ 'act': dduBooking.docAlloc === 'each' }" @click="dduBooking.docAlloc = 'each'">
                        <span class="s5-dal-chip-r"></span>
                        <div><div class="s5-dal-chip-t">Separate per container</div><div class="s5-dal-chip-h">Each container has its own document set</div></div>
                      </div>
                ...
                <div class="s5-upload-step">
                  ...
                  <div class="s5-upload-step-body">
                    ...
                    <div v-if="dduBooking.uploadMode === 'compressed'" class="s5-upload-zone">
                      ...
                      <button v-if="dduBooking.docAlloc !== 'same'" class="s5-afb" @click="addDduRow('compressed')" type="button">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                        Add more
                      </button>
                    </div>
            ...
            <div class="s5-sb"><div class="s5-fg">
              ...
              <div style="display:grid;grid-template-columns:160px 280px 1fr;gap:10px;align-items:start;">
                ...
                <div style="border:1.5px solid var(--s5-border,#e5e7eb);border-radius:10px;padding:10px 14px;">
                  ...
                  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;">
                    <div>
                      <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Demurrage <span style="color:#dc2626;">*</span></div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input type="number" class="s5-fi" v-model="dduBooking.arrFreeDemurrage" placeholder="e.g. 14" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" />
                        <span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span>
                      </div>
                    </div>
                    <div>
                      <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Detention <span style="color:#dc2626;">*</span></div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input type="number" class="s5-fi" v-model="dduBooking.arrFreeDetention" placeholder="e.g. 7" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" />
                        <span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span>
                      </div>
                    </div>
                    <div>
                      <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Combine</div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input type="number" class="s5-fi" v-model="dduBooking.arrFreeCombine" placeholder="e.g. 21" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" />
                        <span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span>
                      </div>
                    </div>
                  </div>
        ...
        <div v-show="activeStep5Tab === 's5-dest-ddp'" class="s5-page">
          ...
          <div class="s5-mc" style="margin-bottom:16px">
            ...
            <div class="s5-sb"><div class="s5-fg">
              ...
              <div class="s5-ff">
                <span class="s5-fl">Receiving company <span class="s5-rq">*</span></span>
                <input type="text" class="s5-fi" :class="{ 's5-disabled': ddpBooking.sameAsConsignee }" v-model="ddpBooking.receivingCompany" :disabled="ddpBooking.sameAsConsignee" placeholder="Enter company name" />
              </div>

              <div class="s5-row-addr-zip">
                <div class="s5-ff">
                  <span class="s5-fl">Company address <span class="s5-rq">*</span></span>
                  <input type="text" class="s5-fi" :class="{ 's5-disabled': ddpBooking.sameAsConsignee }" v-model="ddpBooking.companyAddress" :disabled="ddpBooking.sameAsConsignee" placeholder="Full delivery address including city, state, country" />
                </div>
                <div class="s5-ff">
                  <span class="s5-fl">Zip code <span class="s5-rq">*</span></span>
                  <input type="text" class="s5-fi" :class="{ 's5-disabled': ddpBooking.sameAsConsignee }" v-model="ddpBooking.zipCode" :disabled="ddpBooking.sameAsConsignee" placeholder="e.g. 700000" maxlength="10" />
                </div>
              </div>
              ...
              <div class="s5-row-pic">
                <div class="s5-ff">
                  <span class="s5-fl">Title</span>
                  <select class="s5-fi s5-fi-select custom-caret-svg" :class="{ 's5-disabled': ddpBooking.sameAsConsignee }" v-model="ddpBooking.picTitle" :disabled="ddpBooking.sameAsConsignee">
                    <option>Mr.</option><option>Mrs.</option><option>Ms.</option>
                  </select>
                </div>
                <div class="s5-ff">
                  <span class="s5-fl">PIC name <span class="s5-rq">*</span></span>
                  <input type="text" class="s5-fi" :class="{ 's5-disabled': ddpBooking.sameAsConsignee }" v-model="ddpBooking.picName" :disabled="ddpBooking.sameAsConsignee" placeholder="Contact person in charge" />
                </div>
                <div class="s5-ff">
                  <span class="s5-fl">Email <span class="s5-rq">*</span></span>
                  <input type="text" class="s5-fi" :class="{ 's5-disabled': ddpBooking.sameAsConsignee }" v-model="ddpBooking.email" :disabled="ddpBooking.sameAsConsignee" placeholder="email@company.com" />
                </div>
                <div class="s5-ff">
                  <span class="s5-fl">Phone <span class="s5-rq">*</span></span>
                  <input type="tel" class="s5-fi" :class="{ 's5-disabled': ddpBooking.sameAsConsignee }" v-model="ddpBooking.phone" :disabled="ddpBooking.sameAsConsignee" placeholder="e.g. +84 28 1234 567" maxlength="18" />
                </div>
              </div>
          ...
          <div class="s5-mc" style="margin-bottom:16px">
            ...
            <div class="s5-sb"><div class="s5-fg">
              <div style="display:grid;grid-template-columns:140px 1fr 1.4fr 1.2fr 160px;gap:10px;align-items:end;">
                <div class="s5-ff"><span class="s5-fl">Shipping line</span><select class="s5-fi s5-fi-select custom-caret-svg" v-model="ddpBooking.arrShippingLine"><option value="">Select carrier</option><option>MAERSK</option><option>MSC</option><option>CMA CGM</option><option>COSCO</option><option>HAPAG-LLOYD</option><option>ONE</option><option>EVERGREEN</option><option>HMM</option><option>YANG MING</option><option>ZIM</option><option>PIL</option><option>OOCL</option><option>WAN HAI</option></select></div>
                <div class="s5-ff"><span class="s5-fl">B/L number{{ ddpBooking.blMode === 'house' ? ' (Master)' : '' }}</span><input type="text" class="s5-fi" v-model="ddpBooking.arrBlNumber" placeholder="Input B/L number" /></div>
                <div class="s5-ff"><span class="s5-fl">Vessel name</span><input type="text" class="s5-fi" v-model="ddpBooking.arrVesselName" placeholder="Input vessel name" /></div>
                <div class="s5-ff"><span class="s5-fl">Voyage number</span><input type="text" class="s5-fi" v-model="ddpBooking.arrVoyageNumber" placeholder="Input voyage number" /></div>
                <div class="s5-ff"><span class="s5-fl">Est. Arrival time</span><div class="s5-date-wrap"><EnDatePicker :min-date="minDate" v-model="ddpBooking.arrEstArrival" input-class="s5-fi" /></div></div>
              </div>
              <div style="display:grid;grid-template-columns:160px 280px 1fr;gap:10px;align-items:start;">
                <div class="s5-ff"><span class="s5-fl">Destination charges</span><select class="s5-fi s5-fi-select custom-caret-svg" v-model="ddpBooking.arrDestCharges" style="font-size:11px;"><option>PREPAID / COLLECT</option><option>PREPAID</option><option>COLLECT</option></select></div>
                <div class="s5-ff"><span class="s5-fl">Payment party</span><select class="s5-fi s5-fi-select custom-caret-svg" v-model="ddpBooking.arrPaymentParty"><option>Shoptrans pays on our behalf</option><option>Consignee shall pay the carrier directly</option><template v-if="ddpBooking.blMode === 'house'"><option>Shoptrans pays on consignee's behalf</option></template></select></div>
                <div style="border:1.5px solid var(--s5-border,#e5e7eb);border-radius:10px;padding:10px 14px;"><div class="s5-fl" style="margin-bottom:10px;">Freetime application</div><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;"><div><div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Demurrage</div><div style="display:flex;align-items:center;gap:6px;"><input type="number" class="s5-fi" v-model="ddpBooking.arrFreeDemurrage" placeholder="e.g. 14" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" /><span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span></div></div><div><div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Detention</div><div style="display:flex;align-items:center;gap:6px;"><input type="number" class="s5-fi" v-model="ddpBooking.arrFreeDetention" placeholder="e.g. 7" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" /><span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span></div></div><div><div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Combine</div><div style="display:flex;align-items:center;gap:6px;"><input type="number" class="s5-fi" v-model="ddpBooking.arrFreeCombine" placeholder="e.g. 21" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" /><span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span></div></div></div></div>
              </div>
            </div></div>
          </div>
              ...
              <div v-if="ddpBooking.customsOption === 2" style="margin-top:4px;">
                ...
                <div class="s5-upload-step">
                  ...
                  <div class="s5-upload-step-body">
                    <div class="s5-dal-label">Document Set Allocation</div>
                    <div class="s5-dal-chips">
                      <div class="s5-dal-chip" :class="{ 'act': ddpBooking.docAlloc === 'same' }" @click="ddpBooking.docAlloc = 'same'">
                        <span class="s5-dal-chip-r"></span>
                        <div><div class="s5-dal-chip-t">Same document set</div><div class="s5-dal-chip-h">All containers share one set</div></div>
                      </div>
                      <div class="s5-dal-chip" :class="{ 'act': ddpBooking.docAlloc === 'each' }" @click="ddpBooking.docAlloc = 'each'">
                        <span class="s5-dal-chip-r"></span>
                        <div><div class="s5-dal-chip-t">Separate per container</div><div class="s5-dal-chip-h">Each container has its own document set</div></div>
                      </div>
                ...
                <div class="s5-upload-step">
                  ...
                  <div class="s5-upload-step-body">
                    ...
                    <div v-if="ddpBooking.uploadMode === 'compressed'" class="s5-upload-zone">
                      ...
                      <button v-if="ddpBooking.docAlloc !== 'same'" class="s5-afb" @click="addDdpRow('compressed')" type="button">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                        Add more
                      </button>
                    </div>
                ...
                <div style="border:1.5px solid var(--s5-border,#e5e7eb);border-radius:10px;padding:10px 14px;">
                  ...
                  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;">
                    <div>
                      <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Demurrage <span style="color:#dc2626;">*</span></div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input type="number" class="s5-fi" v-model="ddpBooking.arrFreeDemurrage" placeholder="e.g. 14" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" />
                        <span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span>
                      </div>
                    </div>
                    <div>
                      <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Detention <span style="color:#dc2626;">*</span></div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input type="number" class="s5-fi" v-model="ddpBooking.arrFreeDetention" placeholder="e.g. 7" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" />
                        <span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span>
                      </div>
                    </div>
                    <div>
                      <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Combine</div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input type="number" class="s5-fi" v-model="ddpBooking.arrFreeCombine" placeholder="e.g. 21" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" />
                        <span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span>
                      </div>
                    </div>
                  </div>
                ...
                <div style="border:1.5px solid var(--s5-border,#e5e7eb);border-radius:10px;padding:10px 14px;">
                  ...
                  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;">
                    <div>
                      <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Demurrage <span style="color:#dc2626;">*</span></div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input type="number" class="s5-fi" v-model="destCustomsOnlyBooking.arrFreeDemurrage" placeholder="e.g. 14" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" />
                        <span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span>
                      </div>
                    </div>
                    <div>
                      <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Detention <span style="color:#dc2626;">*</span></div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input type="number" class="s5-fi" v-model="destCustomsOnlyBooking.arrFreeDetention" placeholder="e.g. 7" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" />
                        <span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span>
                      </div>
                    </div>
                    <div>
                      <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;">Combine</div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input type="number" class="s5-fi" v-model="destCustomsOnlyBooking.arrFreeCombine" placeholder="e.g. 21" min="0" step="1" style="width:70px;text-align:center;padding:7px 8px;" />
                        <span style="font-size:11px;font-weight:600;color:#9ca3af;white-space:nowrap;">DAY(S)</span>
                      </div>
                    </div>
                  </div>
        ...
        <div v-show="activeStep5Tab === 's5-dest-dodRelease'" class="s5-page">
          ...
          <!-- ── CHARGES ARRANGEMENT (only when combined with handleUnderAvailable) ── -->
          <div class="s5-mc" style="margin-bottom:16px" v-if="services.sea.selected.includes('handleUnderAvailable')">
            <div class="s5-sd" style="border-radius:14px 14px 0 0">
              <div class="s5-sd-left">
          ...
          <!-- ── FREETIME APPLICATION (only when combined with handleUnderAvailable) ── -->
          <div class="s5-mc" style="margin-bottom:16px" v-if="services.sea.selected.includes('handleUnderAvailable')">
            <div class="s5-sd" style="border-radius:14px 14px 0 0">
              <div class="s5-sd-left">
                <div class="s5-si"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div>
                <span class="s5-st">Freetime application</span>
              </div>
            </div>
            <div class="s5-sb">
              <div style="display:flex;gap:40px;justify-content:center;">
                <div>
                  <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.08em;text-transform:uppercase;text-align:center;margin-bottom:6px;">Demurrage</div>
                  <div style="display:flex;align-items:center;gap:8px;">
                    <input type="number" class="s5-fi" v-model="doReleaseBooking.freeDemurrage" placeholder="—" min="0" step="1" style="width:90px;text-align:center;padding:9px 10px;" />
                    <span style="font-size:11px;font-weight:700;color:#9ca3af;">DAY(S)</span>
                  </div>
                </div>
                <div>
                  <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.08em;text-transform:uppercase;text-align:center;margin-bottom:6px;">Detention</div>
                  <div style="display:flex;align-items:center;gap:8px;">
                    <input type="number" class="s5-fi" v-model="doReleaseBooking.freeDetention" placeholder="—" min="0" step="1" style="width:90px;text-align:center;padding:9px 10px;" />
                    <span style="font-size:11px;font-weight:700;color:#9ca3af;">DAY(S)</span>
                  </div>
                </div>
                <div>
                  <div style="font-size:10px;font-weight:700;color:#6b7280;letter-spacing:.08em;text-transform:uppercase;text-align:center;margin-bottom:6px;">Combine</div>
                  <div style="display:flex;align-items:center;gap:8px;">
                    <input type="number" class="s5-fi" v-model="doReleaseBooking.freeCombine" placeholder="—" min="0" step="1" style="width:90px;text-align:center;padding:9px 10px;" />
                    <span style="font-size:11px;font-weight:700;color:#9ca3af;">DAY(S)</span>
                  </div>
                </div>
              </div>
          ...
          <!-- ── PRE-ALERT (only when combined with handleUnderAvailable) ── -->
          <div class="s5-mc" style="margin-bottom:16px" v-if="services.sea.selected.includes('handleUnderAvailable')">
            <div class="s5-sd" style="border-radius:14px 14px 0 0">
              <div class="s5-sd-left">
                <div class="s5-si"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg></div>
            ...
            <div class="s5-sb" style="padding:0;">
              <div style="display:grid;grid-template-columns:1fr 1px 1fr;">
                ...
                <div style="padding:22px 24px;">
                  ...
                  <div style="display:flex;flex-direction:column;gap:6px;">
                    <template v-for="(f, i) in doReleaseBooking.preAlertMaster" :key="i">
                      <div class="s5-av-chip">
                        <span style="font-size:16px;">📎</span>
                        <span class="s5-av-chip-name">{{ f.name }}</span>
                        <button class="s5-av-chip-del" @click="doReleaseBooking.preAlertMaster.splice(i,1)">×</button>
                      </div>
                    </template>
                    <label class="s5-av-add-btn">
                      <input type="file" style="display:none" multiple accept=".rar,.zip,.7z,.pdf,.xlsx,.xls,.docx,.doc" @change="(e: Event) => { const ft=(e.target as HTMLInputElement).files; if(ft) Array.from(ft).forEach(file => doReleaseBooking.preAlertMaster.push(file)); (e.target as HTMLInputElement).value='' }" />
                      <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                      Add file
                    </label>
                  </div>
                ...
                <div style="padding:22px 24px;">
                  ...
                  <div style="display:flex;flex-direction:column;gap:6px;">
                    <template v-for="(f, i) in doReleaseBooking.preAlertHouse" :key="i">
                      <div class="s5-av-chip">
                        <span style="font-size:16px;">📎</span>
                        <span class="s5-av-chip-name">{{ f.name }}</span>
                        <button class="s5-av-chip-del" @click="doReleaseBooking.preAlertHouse.splice(i,1)">×</button>
                      </div>
                    </template>
                    <label class="s5-av-add-btn">
                      <input type="file" style="display:none" multiple accept=".rar,.zip,.7z,.pdf,.xlsx,.xls,.docx,.doc" @change="(e: Event) => { const ft=(e.target as HTMLInputElement).files; if(ft) Array.from(ft).forEach(file => doReleaseBooking.preAlertHouse.push(file)); (e.target as HTMLInputElement).value='' }" />
                      <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                      Add file
                    </label>
                  </div>
          ...
          <div class="s5-mc s5-mc-col">
            ...
            <div class="s5-sb">
              ...
              <div v-if="destCollectPayBooking.colTab === 'manual'" class="s5-cp-fee-wrap">
                <div class="s5-cp-fee-actions">
                  <button class="s5-cp-fee-btn s5-cp-fee-add" :class="{ 's5-inv': step5Errors.has('dcp.collection.fees') }" @click="step5Errors.delete('dcp.collection.fees'); destCollectPayBooking.colRows.push({ chargeName:'Ocean Freight', unit:'CNT', quantity:'', unitPrice:'', currency:'USD', total:'', receiveFrom:'Shipper' })">
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg> ADD
                  </button>
                </div>
                <div class="s5-cp-table-wrap">
                  <table class="s5-cp-table">
                    ...
                    <tbody>
                      <tr v-for="(row, i) in destCollectPayBooking.colRows" :key="i">
                        <td><select class="s5-cp-tc-sel custom-caret-svg" :class="{ 's5-inv': step5Errors.has('dcp.collection.fees.' + i + '.chargeName') }" v-model="row.chargeName" @change="step5Errors.delete('dcp.collection.fees.' + i + '.chargeName')"><option v-for="cn in cpChargeNameOpts" :key="cn" :value="cn">{{ cn }}</option></select></td>
                        <td><select class="s5-cp-tc-sel custom-caret-svg" :class="{ 's5-inv': step5Errors.has('dcp.collection.fees.' + i + '.unit') }" v-model="row.unit" @change="step5Errors.delete('dcp.collection.fees.' + i + '.unit')"><option v-for="u in cpUnitOpts" :key="u" :value="u">{{ u }}</option></select></td>
                        <td><input type="number" class="s5-cp-tc" :class="{ 's5-inv': step5Errors.has('dcp.collection.fees.' + i + '.quantity') }" v-model="row.quantity" @input="step5Errors.delete('dcp.collection.fees.' + i + '.quantity')" placeholder="1" min="0.01" step="any" /></td>
                        <td><input type="number" class="s5-cp-tc" :class="{ 's5-inv': step5Errors.has('dcp.collection.fees.' + i + '.unitPrice') }" v-model="row.unitPrice" @input="step5Errors.delete('dcp.collection.fees.' + i + '.unitPrice')" placeholder="0.00" min="0" step="0.01" /></td>
                        <td><select class="s5-cp-tc-sel custom-caret-svg" :class="{ 's5-inv': step5Errors.has('dcp.collection.fees.' + i + '.currency') }" v-model="row.currency" @change="step5Errors.delete('dcp.collection.fees.' + i + '.currency')"><option>USD</option><option>EUR</option><option>VND</option><option>CNY</option><option>JPY</option><option>GBP</option></select></td>
                        <td><div class="s5-cp-tc-total">{{ row.quantity && row.unitPrice ? (parseFloat(row.quantity)*parseFloat(row.unitPrice)).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+' '+row.currency : '—' }}</div></td>
                        <td><select class="s5-cp-tc-sel custom-caret-svg" v-model="row.receiveFrom"><option>Shipper</option><option>3rd Party</option></select></td>
              ...
              <div v-if="destCollectPayBooking.colTab !== 'later'" class="s5-cp-pic-section s5-cp-pic-green">
                ...
                <div v-for="(pic, pi) in dcpColPics" :key="'dcpic-c-'+pi" class="s5-cp-pic-row" :style="pi > 0 ? 'margin-top:8px' : ''">
                  ...
                  <div class="s5-cp-pic-field">
                    <label class="s5-cp-pic-fl">PIC name</label>
                    <input class="s5-cp-pic-fi" :class="{ 's5-inv': step5Errors.has('dcp.collection.pics.' + pi + '.name') }" type="text" v-model="pic.name" @input="step5Errors.delete('dcp.collection.pics.' + pi + '.name')" placeholder="Input PIC name" />
                  </div>
                  <div class="s5-cp-pic-field">
                    <label class="s5-cp-pic-fl">Email</label>
                    <input class="s5-cp-pic-fi" :class="{ 's5-inv': step5Errors.has('dcp.collection.pics.' + pi + '.email') }" type="email" v-model="pic.email" @input="step5Errors.delete('dcp.collection.pics.' + pi + '.email')" placeholder="Input Email address" />
                  </div>
                  <div class="s5-cp-pic-field">
                    <label class="s5-cp-pic-fl">Phone</label>
                    <input class="s5-cp-pic-fi" :class="{ 's5-inv': step5Errors.has('dcp.collection.pics.' + pi + '.phone') }" type="tel" v-model="pic.phone" @input="step5Errors.delete('dcp.collection.pics.' + pi + '.phone')" placeholder="Input phone number" maxlength="20" />
                  </div>
                  <div class="s5-cp-pic-field s5-cp-pic-btn-cell">
          ...
          <div class="s5-mc s5-mc-pay">
            ...
            <div class="s5-sb">
              ...
              <div v-if="destCollectPayBooking.payTab === 'manual'" class="s5-cp-fee-wrap">
                <div class="s5-cp-fee-actions">
                  <button class="s5-cp-fee-btn s5-cp-fee-add" :class="{ 's5-inv': step5Errors.has('dcp.payment.fees') }" @click="step5Errors.delete('dcp.payment.fees'); destCollectPayBooking.payRows.push({ chargeName:'Ocean Freight', unit:'CNT', quantity:'', unitPrice:'', currency:'USD', total:'', payTo:'Shipper' })">
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg> ADD
                  </button>
                </div>
                <div class="s5-cp-table-wrap">
                  <table class="s5-cp-table">
                    ...
                    <tbody>
                      <tr v-for="(row, i) in destCollectPayBooking.payRows" :key="i">
                        <td><select class="s5-cp-tc-sel custom-caret-svg" :class="{ 's5-inv': step5Errors.has('dcp.payment.fees.' + i + '.chargeName') }" v-model="row.chargeName" @change="step5Errors.delete('dcp.payment.fees.' + i + '.chargeName')"><option v-for="cn in cpChargeNameOpts" :key="cn" :value="cn">{{ cn }}</option></select></td>
                        <td><select class="s5-cp-tc-sel custom-caret-svg" :class="{ 's5-inv': step5Errors.has('dcp.payment.fees.' + i + '.unit') }" v-model="row.unit" @change="step5Errors.delete('dcp.payment.fees.' + i + '.unit')"><option v-for="u in cpUnitOpts" :key="u" :value="u">{{ u }}</option></select></td>
                        <td><input type="number" class="s5-cp-tc" :class="{ 's5-inv': step5Errors.has('dcp.payment.fees.' + i + '.quantity') }" v-model="row.quantity" @input="step5Errors.delete('dcp.payment.fees.' + i + '.quantity')" placeholder="1" min="0.01" step="any" /></td>
                        <td><input type="number" class="s5-cp-tc" :class="{ 's5-inv': step5Errors.has('dcp.payment.fees.' + i + '.unitPrice') }" v-model="row.unitPrice" @input="step5Errors.delete('dcp.payment.fees.' + i + '.unitPrice')" placeholder="0.00" min="0" step="0.01" /></td>
                        <td><select class="s5-cp-tc-sel custom-caret-svg" :class="{ 's5-inv': step5Errors.has('dcp.payment.fees.' + i + '.currency') }" v-model="row.currency" @change="step5Errors.delete('dcp.payment.fees.' + i + '.currency')"><option>USD</option><option>EUR</option><option>VND</option><option>CNY</option><option>JPY</option><option>GBP</option></select></td>
                        <td><div class="s5-cp-tc-total">{{ row.quantity && row.unitPrice ? (parseFloat(row.quantity)*parseFloat(row.unitPrice)).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+' '+row.currency : '—' }}</div></td>
                        <td>
                          <span v-if="['Customs Fee','Inspection Fee','Fumigation Fee'].includes(row.chargeName)" class="s5-cp-locked">🔒 3rd Party</span>
                          <select v-else class="s5-cp-tc-sel custom-caret-svg" v-model="row.payTo"><option>Shipper</option><option>3rd Party</option></select>
              ...
              <div v-if="destCollectPayBooking.payTab !== 'later'" class="s5-cp-pic-section s5-cp-pic-red">
                ...
                <div v-for="(pic, pi) in dcpPayPics" :key="'dcpic-p-'+pi" class="s5-cp-pic-row" :style="pi > 0 ? 'margin-top:8px' : ''">
                  ...
                  <div class="s5-cp-pic-field">
                    <label class="s5-cp-pic-fl">PIC name</label>
                    <input class="s5-cp-pic-fi" :class="{ 's5-inv': step5Errors.has('dcp.payment.pics.' + pi + '.name') }" type="text" v-model="pic.name" @input="step5Errors.delete('dcp.payment.pics.' + pi + '.name')" placeholder="Input PIC name" />
                  </div>
                  <div class="s5-cp-pic-field">
                    <label class="s5-cp-pic-fl">Email</label>
                    <input class="s5-cp-pic-fi" :class="{ 's5-inv': step5Errors.has('dcp.payment.pics.' + pi + '.email') }" type="email" v-model="pic.email" @input="step5Errors.delete('dcp.payment.pics.' + pi + '.email')" placeholder="Input Email address" />
                  </div>
                  <div class="s5-cp-pic-field">
                    <label class="s5-cp-pic-fl">Phone</label>
                    <input class="s5-cp-pic-fi" :class="{ 's5-inv': step5Errors.has('dcp.payment.pics.' + pi + '.phone') }" type="tel" v-model="pic.phone" @input="step5Errors.delete('dcp.payment.pics.' + pi + '.phone')" placeholder="Input phone number" maxlength="20" />
                  </div>
                  <div class="s5-cp-pic-field s5-cp-pic-btn-cell">
          ...
          <div class="s5-mc">
            ...
            <template v-if="destInsuranceBooking.submitType==='NOW'">
              ...
              <div class="s5-ins-section">
                ...
                <!-- Checklist -->
                <div class="s5-dcl" :class="destInsuranceBooking.ensureDocSet ? 'chk' : ''" @click="destInsuranceBooking.ensureDocSet=!destInsuranceBooking.ensureDocSet">
                  <span class="s5-dcb"></span>
...
import { computed, inject, ref, reactive, watch, nextTick } from 'vue'
import type { Ref, ComputedRef } from 'vue'

// Inject all shared state from parent
const services = inject('fcl-services') as any
const shipperInfo = inject('fcl-shipperInfo') as any
const consigneeInfo = inject('fcl-consigneeInfo') as any
const fcaBooking = inject('fcl-fcaBooking') as any
const exwBooking = inject('fcl-exwBooking') as any
const customsBooking = inject('fcl-customsBooking') as any
const collectPayBooking = inject('fcl-collectPayBooking') as any
const insuranceBooking = inject('fcl-insuranceBooking') as any
const insCompEditIdx = ref(-1)
const insSepEditIdx = ref(-1)
const destInsuranceBooking = inject('fcl-destInsuranceBooking') as any
const destInsCompEditIdx = ref(-1)
const destInsSepEditIdx = ref(-1)
const offeredBooking = inject('fcl-offeredBooking') as any
const spotBooking = inject('fcl-spotBooking') as any
const availableBooking = inject('fcl-availableBooking') as any
const contractBooking = inject('fcl-contractBooking') as any
const switchBillBooking = inject('fcl-switchBillBooking') as any
const sblEditingIdx = ref(-1)
const dapBooking = inject('fcl-dapBooking') as any
const dduBooking = inject('fcl-dduBooking') as any
const ddpBooking = inject('fcl-ddpBooking') as any
const customsOnlyBooking = inject('fcl-customsOnlyBooking') as any
const destCustomsOnlyBooking = inject('fcl-destCustomsOnlyBooking') as any
const destCollectPayBooking = inject('fcl-destCollectPayBooking') as any


const step5TabList = inject('fcl-step5TabList') as ComputedRef<any[]>
const activeStep5Tab = inject('fcl-activeStep5Tab') as Ref<string>
const step5Errors = inject('fcl-step5Errors') as Set<string>

const validateStep5 = inject('fcl-validateStep5') as () => void
const goBackStep5Tab = inject('fcl-goBackStep5Tab') as () => void
...
const doReleaseBooking = inject('fcl-doReleaseBooking') as any
const changeOptionGoToStep2 = inject('fcl-changeOptionGoToStep2') as () => void
const minDate = inject('fcl-minDate') as ComputedRef<string>
...
const buildSeparateDefaults = (alloc: string): UploadRow[] => {
  if (alloc === 'same') {
    return [
      ...
      { name: 'Packing_List', icon: '📋', required: true, file: null },
      { name: 'Sales_Contract', icon: '📝', required: true, file: null },
    ]
  } else if (alloc === 'each') {
...
const resetDapRows = () => {
  ...
  dapCompressedRows.splice(0, dapCompressedRows.length, ...buildCompressedDefaults(alloc))
  dapSeparateRows.splice(0, dapSeparateRows.length, ...buildSeparateDefaults(alloc))
}
resetDapRows()
...
const resetDduRows = () => {
  ...
  dduCompressedRows.splice(0, dduCompressedRows.length, ...buildCompressedDefaults(alloc))
  dduSeparateRows.splice(0, dduSeparateRows.length, ...buildSeparateDefaults(alloc))
}
resetDduRows()
...
const resetDdpRows = () => {
  ...
  ddpCompressedRows.splice(0, ddpCompressedRows.length, ...buildCompressedDefaults(alloc))
  ddpSeparateRows.splice(0, ddpSeparateRows.length, ...buildSeparateDefaults(alloc))
}
resetDdpRows()

watch(() => ddpBooking.docAlloc, () => { resetDdpRows() })
...
const resetDcoRows = () => {
  ...
  dcoCompressedRows.splice(0, dcoCompressedRows.length, ...buildCompressedDefaults(alloc))
  dcoSeparateRows.splice(0, dcoSeparateRows.length, ...buildSeparateDefaults(alloc))
}
resetDcoRows()

watch(() => destCustomsOnlyBooking.docAlloc, () => { resetDcoRows() })
...
const createCpPicRow = (): CpPicRow => ({ title: 'Mr.', name: '', email: '', phone: '' })
const ensureCpPicRows = (booking: any, key: string): CpPicRow[] => {
  if (!Array.isArray(booking[key]) || booking[key].length === 0) {
    booking[key] = [createCpPicRow()]
  }
  return booking[key]
}
...
const cpColPics = computed(() => ensureCpPicRows(collectPayBooking, 'collectionPics'))
const cpPayPics = computed(() => ensureCpPicRows(collectPayBooking, 'paymentPics'))
...
const dcpColFileRows = reactive<CpFileRow[]>([{ name: 'Document_1', file: null }])
const dcpPayFileRows = reactive<CpFileRow[]>([{ name: 'Document_1', file: null }])
const dcpEditingFile = ref('')
const dcpFileInputRefs = reactive<Record<string, HTMLInputElement>>({})

const dcpColPics = computed(() => ensureCpPicRows(destCollectPayBooking, 'colPics'))
const dcpPayPics = computed(() => ensureCpPicRows(destCollectPayBooking, 'payPics'))
...
.s5-row-ship { display: grid; grid-template-columns: 180px 1fr 1.3fr 1fr 156px; gap: 10px; align-items: end; }
...
.s5-mc-col { margin-bottom: 16px; overflow: hidden; }
.s5-mc-pay { border-color: #fecaca; margin-bottom: 16px; overflow: hidden; }
...
.s5-sd-red { display: flex; align-items: center; justify-content: space-between; gap: 10px; padding: 14px 24px; background: linear-gradient(135deg, #fff5f5, #fff); border-bottom: 1px solid #fecaca; border-radius: 14px 14px 0 0; }
