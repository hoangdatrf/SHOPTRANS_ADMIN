...
export default defineEventHandler(async (event) => {
  const query = getQuery(event); // Lấy query parameters
  const objCode = query.objCode;
  const orgCode = query.orgCode;
  const outputPath = query.outputPath;

  if (!objCode) {
    throw createError({
      statusCode: 400,
      statusMessage: 'Please provide a object code',
    });
  }
  if (!orgCode) {
    throw createError({
      statusCode: 400,
      statusMessage: 'Please provide a org code',
    });
  }

  await getCognosLogOff(); // Gọi logoff trước
  const result = await generateReport(objCode, orgCode, outputPath); // Gọi hàm generateReport
