  ...
  <div class="admin-main">
        ...
        <template v-if="pageSection === 'fcl'">
          <div class="fcl-shell">
            <div class="fcl-data-tabs" aria-label="FCL page data sections">
              ...
              <button type="button" :class="{ active: fclDataSection === 'cargo', done: fclDataSection === 'shipping' }" @click="setFclDataSection('cargo')">
                <strong>
                  <svg v-if="fclDataSection === 'shipping'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
                  <span v-else>2</span>
                </strong>
                <span>What do you want to ship?</span>
                <small>{{ fclCommodities.length }} rows</small>
              </button>
              <button type="button" :class="{ active: fclDataSection === 'shipping' }" @click="setFclDataSection('shipping')">
            ...
            <div class="dataset-toolbar">
              ...
              <div class="dataset-actions">
                ...
                <button class="line-btn icon-text-btn" type="button" @click="openCreateFclModal">
        ...
        <div class="modal-head">
          <div>
            ...
            <h2 id="fcl-dialog-title">
              {{ fclDataSection === 'locations' ? 'Location Data' : fclDataSection === 'cargo' ? 'Commodity Data' : 'Container Data' }}
            </h2>
          </div>
    ...
    <div v-if="fclDeleteTarget" class="modal-backdrop subtle-backdrop" role="presentation" @click.self="closeDeleteFclDialog">
      <section class="delete-dialog" role="dialog" aria-modal="true" aria-labelledby="delete-fcl-title">
        ...
        <div>
          <h2 id="delete-fcl-title">Delete this {{ fclDeleteTarget.section === 'locations' ? 'location' : fclDeleteTarget.section === 'cargo' ? 'commodity' : 'container' }}?</h2>
        ...
        <div class="hs-importer-body">
          ...
          <div class="hs-importer-controls">
            ...
            <button class="ghost-btn icon-text-btn" type="button" :disabled="hsLoading" @click="fetchHsCodes">
              ...
              {{ hsLoading ? 'Đang tải...' : 'Tải dữ liệu WCO' }}
            </button>
          ...
          <div class="hs-table-wrap">
            ...
            <AdminLoadingScreen v-else-if="hsLoading" compact message="Loading data from WCO" />
...
type FclDataSection = 'locations' | 'cargo' | 'shipping'
...
const duplicateFclRecord = (item: FclLocationRecord | FclCommodityRecord | FclContainerRecord) => {
  activeFclActionKey.value = ''
...
const fetchHsCodes = async () => {
  ...
  hsSelectedItems.value = []
  try {
    const url = 'https://raw.githubusercontent.com/datasets/harmonized-system/master/data/harmonized-system.csv'
    ...
    for (const line of lines) {
      ...
      // CSV format: section,hscode,description,parent,level
      const parts = line.match(/^[^,]+,(\d+),"?([^"]+)"?,\w+,(\d+)/)
      if (!parts) continue
      const [, hscode, description, level] = parts
      if (level !== '6' || hscode.length !== 6) continue
      items.push({
        hscode,
        description: description.trim(),
        exists: existingCodes.has(hscode)
      })
    }
    hsMappedItems.value = items
    if (!items.length) hsError.value = 'Không phân tích được dữ liệu CSV. Kiểm tra lại kết nối mạng.'
  } catch (e: any) {
    hsError.value = `Không thể tải dữ liệu: ${e.message || e}. Đảm bảo trình duyệt cho phép kết nối đến GitHub CDN.`
  } finally {
...
const toggleFclEnabled = (item: FclLocationRecord | FclCommodityRecord | FclContainerRecord) => {
...
const currentFclTotalRows = computed(() => {
  ...
  return filteredFclContainers.value.length
})
